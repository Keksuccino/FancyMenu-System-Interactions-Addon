...
public class OptInScreen extends Screen {
    ...
    private final Component headline = Components.translatable("fmsia.optin.screen.text.headline").withStyle(Style.EMPTY.withBold(true));
    private ScrollableMarkdownRenderer markdownRenderer;
    private final Font font = Minecraft.getInstance().font;

    public OptInScreen(@Nullable Screen parent) {
        super(Components.translatable("fmsia.optin.screen.title"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        this.addRenderableWidget(new ExtendedButton(centerX - 5 - 150, this.height - (FOOTER_HEIGHT / 2) - 10, 150, 20, Components.translatable("fmsia.optin.screen.enable").withStyle(ChatFormatting.GREEN), button -> {
            try {
        ...
        this.addRenderableWidget(new ExtendedButton(centerX + 5, this.height - (FOOTER_HEIGHT / 2) - 10, 150, 20, Components.translatable("fmsia.optin.screen.disable").withStyle(ChatFormatting.RED), button -> {
            try {
    ...
    public void renderBackground(@NotNull PoseStack graphics) {
        ...
        this.fillGradient(graphics, 0, HEADER_HEIGHT, this.width, HEADER_HEIGHT + 4, -16777216, 0);
        this.fillGradient(graphics, 0, this.height - FOOTER_HEIGHT - 4, this.width, this.height - FOOTER_HEIGHT, 0, -16777216);
        RenderingUtils.resetShaderColor();
